...
export default function Tickets() {
  ...
  const handleCreateTicket = (data: TicketFormData) => {
    createTicketMutation.mutate(data, {
      onSuccess: () => {
        toast({ title: 'Ticket created', description: 'A new ticket has been successfully created.' });
        setIsFormOpen(false);
      },
      onError: (error: any) => {
        toast({ title: 'Error', description: error?.message || 'Failed to create ticket.' });
      },
    });
  };
  ...
  const handleDeleteTicket = () => {
    ...
    deleteTicketMutation.mutate(deletingTicket.id, {
      ...
      onError: (error: any) => {
        toast({ title: 'Error', description: error?.message || 'Failed to delete ticket.' });
      },
    });
  };
  ...
  const handleExport = () => {
    ...
    const csvContent = [
      headers.join(','),
      ...tickets.map(t =>
        [`"${t.subject.replace(/"/g, '""')}"`, `"${t.description?.replace(/"/g, '""')}"`, t.category, t.priority, t.status, new Date(t.createdAt).toISOString()].join(',')
      ),
    ].join('\n');
  ...
  if (isLoading) return <div className="p-10 text-muted-foreground">Loading tickets…</div>;
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredTickets.map(ticket => (
              <TicketCard
                ...
                onDelete={t => { setDeletingTicket(t); setIsDeleteDialogOpen(true); }}
      ...
      <TicketForm
        open={isFormOpen}
        onOpenChange={open => { setIsFormOpen(open); if (!open) setEditingTicket(null); }}
